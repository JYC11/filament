...
## Task 6: TUI Integration of Informational Features

**Problem:** The TUI has 5 tabs (Tasks, Agents, Reservations, Messages, Graph) but
none of the Phase 7 informational features are accessible from the TUI. Config,
analytics, lessons, and search are CLI-only.

**Design:**

Add 3 new tabs to the TUI. The current 5 tabs are already fairly dense — adding
every feature would clutter the UI. Group by use case:

### Tab 6: Config (key `6`)

Read-only display of resolved configuration values. No interaction needed.
...
│ context_depth           2              default     │
│ max_auto_dispatch       3              config      │
│ cleanup_interval_secs   60             default     │
│ output_format           text           default     │
└───────────────────────────────────────────────────┘
```

Data source: `FilamentConfig::load()`. No refresh needed (config is static).

### Tab 7: Analytics (key `7`)

Combined PageRank + degree centrality in a split view. Top pane shows PageRank
scores, bottom pane shows degree centrality. Both are read from the graph.
...
│ dispatch-module          0.098712                  │
│ task-management          0.087654                  │
│                                                   │
│ Degree Centrality                                 │
│ ───────────────────────────────────────────────── │
│ Name                     In   Out  Total          │
│ filament-core             8     3     11          │
│ dispatch-module           5     4      9          │
└───────────────────────────────────────────────────┘
```

Data source: `conn.pagerank()` and `conn.degree_centrality()`. Refresh on tab switch
or `r` key (these are computed from the in-memory graph, not stored).

### Tab 8: Lessons (key `8`)

Lesson list with structured fields shown in a detail pane when selected.
Top: lesson list table (name, pattern, priority, status).
Bottom: selected lesson's problem/solution/learned fields.

```
┌─ Lessons ─────────────────────────────────────────┐
│ Name                    Pattern          P  Status │
│ ───────────────────────────────────────────────── │
│ SQLite CHECK gotcha     schema-migration 2  open  │
│ N+1 query fix           n-plus-one       2  open  │
│ thiserror v2 source     thiserror-v2     2  open  │
│                                                   │
│ ─── Selected ──────────────────────────────────── │
│ Problem:  Cannot ALTER TABLE to modify CHECK...   │
│ Solution: Recreate table with new constraint...   │
│ Learned:  SQLite CHECK constraints are immutable  │
└───────────────────────────────────────────────────┘
```

Data source: `conn.list_lessons()`. j/k navigation selects a lesson and updates
the detail pane.

**Implementation:**

Files touched:
- `crates/filament-tui/src/app.rs` — extend `Tab` enum (8 variants), add data fields
  for config, analytics, lessons
- `crates/filament-tui/src/ui.rs` — add render branches for new tabs
- `crates/filament-tui/src/views/config.rs` — new view file
- `crates/filament-tui/src/views/analytics.rs` — new view file
- `crates/filament-tui/src/views/lessons.rs` — new view file
- `crates/filament-tui/src/views/mod.rs` — add modules
- `crates/filament-tui/src/event.rs` — add key bindings 6/7/8

**Refresh strategy:**
- Config: load once on startup, no refresh
- Analytics: refresh on tab switch + manual `r` (expensive graph computation)
- Lessons: refresh on auto-tick like other data tabs

**What NOT to add as TUI tabs:**
- Watch — already a streaming CLI command, doesn't fit table model
- Seed — a one-shot ingest command, not a dashboard view
- Audit — a one-shot git operation, not a dashboard view
- Search — would need a text input widget (ratatui doesn't have one built-in);
  defer to CLI. Could add later with `tui-input` crate if demand exists.
