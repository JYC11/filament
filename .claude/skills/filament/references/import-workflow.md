...
```bash
filament add <crate-name> --type module --summary "..." [--content path/to/README.md]
```

Create ownership relations:

```bash
filament relate project-root owns <crate-name>
```

## Step 3: Import Documentation

```bash
filament add <doc-name> --type doc --summary "..." --content path/to/doc.md \
...
```bash
filament task add <task-name> --summary "..." --priority N
filament task add <dependent> --summary "..." --depends-on <task-name>
```

Or use `--blocks`:

```bash
filament task add <blocker> --summary "..." --blocks <blocked-task>
```

## Step 5: Mark Completed Work

```bash
filament task close <completed-task>
```
...
filament task ready                        # what's unblocked
filament context --around <central-entity> --depth 2
```
