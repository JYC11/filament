...
|-------|-----------|-----------|-----|
| Entity name | Yes → `type-N` | | Could contain project names |
| Summary | Yes → generic | | Could contain project details |
| Key facts | Yes → structure only | | Could contain secrets/details |
| Content path | Yes → NULL | | File paths leak project structure |
| Message body | Yes → generic | | Could contain anything |
| Relation summary | Yes → generic | | Could contain details |
